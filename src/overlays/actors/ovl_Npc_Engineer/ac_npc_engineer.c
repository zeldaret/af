...
void aNEG_actor_ct(Actor* thisx, Game_Play* game_play) {
    ...
    if (gCommonData.unk_1004C->unk_BC(thisx, game_play) == 1) {
        this->unk7C0 = none_proc1;
        gCommonData.unk_1004C->unk_C0(thisx, game_play, &D_809AEFA4_jp);
        this->unk80C = -1;
        this->unk7FD = 0;
        this->unk8AC = -1;
...
void aNEG_actor_save(Actor* thisx, Game_Play* game_play) {
    gCommonData.unk_1004C->unk_C8(thisx, game_play);
}

void aNEG_actor_dt(Actor* thisx, Game_Play* game_play) {
    gCommonData.unk_1004C->unk_C4(thisx, game_play);
}

void aNEG_actor_init(Actor* thisx, Game_Play* game_play) {
    gCommonData.unk_1004C->unk_CC(thisx, game_play);
}
