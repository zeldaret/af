...
s32 cKF_SkeletonInfo_R_play(SkeletonInfoR* skeletonInfo) {
    ...
    if (IS_ZERO(skeletonInfo->morphCounter)) {
        ...
        // A positive morphCounter will wait to gamePlay the animation until the morph has finished.
        ...
        // A negative morphCounter will gamePlay the animation and morph at the same time.
        cKF_SkeletonInfo_R_morphJoint(skeletonInfo);
 ...
 */
void cKF_Si3_draw_SV_R_child(Game_Play* gamePlay, SkeletonInfoR* skeletonInfo, s32* jointIndex,
    ...
    if ((beforeCallback == NULL) ||
        (beforeCallback != NULL && beforeCallback(gamePlay, skeletonInfo, *jointIndex, &newDlist, &displayBufferFlag,
    ...
    if (afterCallback != NULL) {
        afterCallback(gamePlay, skeletonInfo, *jointIndex, &newDlist, &displayBufferFlag, arg, &rotation, &translation);
    }

    (*jointIndex)++;

    for (i = 0; i < jointElem->numberOfChildren; i++) {
        cKF_Si3_draw_SV_R_child(gamePlay, skeletonInfo, jointIndex, beforeCallback, afterCallback, arg, mtx);
    }
 ...
 */
void cKF_Si3_draw_R_SV(Game_Play* gamePlay, SkeletonInfoR* skeletonInfo, Mtx* mtx, DrawCallback beforeCallback,
