...
void mEnv_ChangeRGBLight(u8* dst, u8* srcW0, u8* srcW1, f32 f0) {
    ...
    for (i = 0; i < 3; i++) {
        dst[0] = (s32)(((*srcW1 - *srcW0) * f0 + *srcW0) * l_mEnv_change_weather_env_rate_electric_light);
...
void mEnv_ChangeDiffuseVctlSet(BaseLight* baseLight) {
    ...
    if (common_data.save.sceneNo == SCENE_CONVENI || common_data.save.sceneNo == SCENE_SUPER || common_data.save.sceneNo == SCENE_DEPART || common_data.save.sceneNo == SCENE_DEPART_2 ||
        ...
        switch (common_data.fieldDrawType) {
            ...
            default:
                ...
                dir.z = 60 * cos_s(secondsRadAngle); // not sure why 60.0f doesn't work here
                break;
        }
...
void mEnv_ChangeDiffuseLight(BaseLight* dst, BaseLight* srcW0, BaseLight* srcW1, f32 f0) {
    ...
    for (i = 0; i < 3; i++) { 
        current[i] = (s32)(srcW0->sunColor[i] * l_mEnv_sun_percent_electric_light);
        current2[i] = (s32)(srcW1->sunColor[i] * l_mEnv_sun_percent_electric_light);
    }
    mEnv_ChangeRGBLight(dst->sunColor, current, current2, f0);

    
    for (i = 0; i < 3; i++) { 
        current[i] = (s32)(srcW0->moonColor[i] * l_mEnv_sun_percent_electric_light);
        current2[i] = (s32)(srcW1->moonColor[i] * l_mEnv_sun_percent_electric_light);
    }
    
    mEnv_ChangeRGBLight(dst->moonColor, srcW0->moonColor, srcW1->moonColor, f0);
}
void mEnv_ChangeFogLight(BaseLight* dst, BaseLight* srcW0, BaseLight* srcW1, f32 f0) {
    mEnv_ChangeRGBLight(dst->fogColor, srcW0->fogColor, srcW1->fogColor, f0);

    dst->fogNear = (((srcW1->fogNear & 0x3FF) - (srcW0->fogNear & 0x3FF)) * f0) + (srcW0->fogNear & 0x3FF);
    dst->fogFar = (((srcW1->fogFar) - (srcW0->fogFar)) * f0) + srcW0->fogFar;
}
...
void mEnv_CalcSetLight(BaseLight* dst, BaseLight* srcW0, BaseLight* srcW1, f32 f0) {
    mEnv_ChangeRGBLight(dst->ambientColor, srcW0->ambientColor, srcW1->ambientColor, f0);
    mEnv_CalcSetLight_train(dst);
    mEnv_ChangeDiffuseLight(dst, srcW0, srcW1, f0);
    mEnv_ChangeFogLight(dst, srcW0, srcW1, f0);

    mEnv_ChangeRGBLight(dst->shadowColor, srcW0->shadowColor, srcW1->shadowColor, f0);
    mEnv_ChangeRGBLight(dst->roomColor, srcW0->roomColor, srcW1->roomColor, f0);
    mEnv_ChangeRGBLight(dst->sunColorWindow, srcW0->sunColorWindow, srcW1->sunColorWindow, f0);
    mEnv_ChangeRGBLight(dst->moonColorWindow, srcW0->moonColorWindow, srcW1->moonColorWindow, f0);
    mEnv_ChangeRGBLight(dst->bgColor, srcW0->bgColor, srcW1->bgColor, f0);
}
...
void mEnv_SetFog(Kankyo* kankyo, Global_light* gLight) {
    ...
    if (light->fogNear + info->fogNear <= mEnv_FOG_NEAR_MAX) {
        gLight->fogNear = light->fogNear + info->fogFar;
    } else {
...
#define mEnv_MAKE_WEATHER_TABLE_ENTRY(clear, rain, thunder, snow, blizzard, sakura, heavy_sakura)   \
    ((((clear)&0xF) << 24) | (((rain)&0xF) << 16) | (((thunder)&0xF) << 20) | (((snow)&0xF) << 8) | \
     (((blizzard)&0xF) << 12) | (((sakura)&0xF) << 0) | (((heavy_sakura)&0xF) << 4))
...
void mEnv_RandomWeather(s16* next_weather, s16* next_intensity) {
    static const u32 weather_table[] = {
        ...
        mEnv_MAKE_WEATHER_TABLE_ENTRY(7, 0, 0, 2, 1, 0,
                                      0),  // 70% chance of clear weather, 20% chance of snow, 10% chance of blizzard
        ...
        mEnv_MAKE_WEATHER_TABLE_ENTRY(
            5, 3, 2, 0, 0, 0, 0),  // 50% chance of clear weather, 30% chance of rain, 20% chance of thunderstorms
...
s16 mEnv_GetWindAngleS() { return mEnv_wind_info.windAngleS; }
 
f32 mEnv_GetWindPowerF(void) { return mEnv_wind_info.windPowerF; }
...
void mEnv_TaimatuPointLightWaveMoveProc(Game_Play* play) {
    ...
    if (l_mEnv_point_light_is_flame_electric_light) {
        ...
        u8 c =  (s32)(((wave >= 0.0f ? sin_s(point_light_wave_counter) : -sin_s(point_light_wave_counter)) * 40.0f) + 40.0f);
...
void mEnv_PointLightSet(Game_Play* play, f32 f0){
    ...
    Light_point_ct(&play->kankyo.pointLight, player->actor.world.pos.x, player->actor.world.pos.y,player->actor.world.pos.z - 25.0f, 255,255,220, (*KKData * (1.0f - f0)));
}
...
void mEnv_MakeWindowLightAlpha(s32 enabled) {
    ...
    if (common_data.fieldDrawType == FIELD_DRAW_TYPE_INDOORS) {
        ...
        if (common_data.save.sceneNo == SCENE_MY_ROOM_S || common_data.save.sceneNo == SCENE_MY_ROOM_M ||
            ...
            if(common_data.time.nowSec >= mTM_TIME_TO_SEC(5,0,0) && common_data.time.nowSec < mTM_TIME_TO_SEC(18,0,0)){
                target = 1.0f;
            }
            else{
                target = 0.0f;
        ...
        else if(common_data.time.nowSec >= mTM_TIME_TO_SEC(5,0,0) && common_data.time.nowSec < mTM_TIME_TO_SEC(18,0,0)){
                target = 1.0f;
        }
