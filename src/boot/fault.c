...
// Bss put before fault.h as a hacky way to bypass bss reordering
struct FaultMgr* sFaultInstance;
...
// TODO: Should be in a proper header somewhere else?
...
void fault_ClientRunTask(FaultClientTask* task) {
    ...
    if (sFaultInstance->clientThreadSp != NULL) {
        ...
        osCreateThread(thread, M_THREAD_ID_FAULT, fault_ClientProcessThread, task, sFaultInstance->clientThreadSp,
                       M_PRIORITY_FAULT_CLIENT);
...
void fault_Wait5Seconds(void) {
    ...
    do {
        fault_Sleep(1000 / 60);
    } while ((osGetTime() - start) < OS_USEC_TO_CYCLES(5000000) + 1);
...
void fault_WaitForButtonCombo(void) {
    ...
    while (state != 11) {
        ...
        if (curBtn == 0 && s1 == s2) {
            ...
            switch (state) {
                case 0:
                    if (curBtn == (Z_TRIG | L_TRIG | R_TRIG) && pressedBtn == Z_TRIG) {
                        state = s2;
                        s1 = s2;
                    }
                    break;
                ...
                case 9:
                    if (pressedBtn == (A_BUTTON | B_BUTTON)) {
                        state = 10;
                    } else if (pressedBtn == A_BUTTON) {
                        state = 0x5B;
                    } else if (pressedBtn == B_BUTTON) {
                        state = 0x5C;
                    } else {
                        state = 0;
                    }
...
void fault_DrawMemDump(uintptr_t pc, uintptr_t sp, uintptr_t cLeftJump, uintptr_t cRightJump) {
    ...
    do {
        ...
        // Move to next page
        if (CHECK_BTN_ALL(input->press.button, START_BUTTON) || CHECK_BTN_ALL(input->cur.button, A_BUTTON)) {
            return;
        }
        ...
        if (CHECK_BTN_ALL(input->cur.button, Z_TRIG)) {
            off *= 0x10;
        }
...
void fault_AddHungupAndCrashImpl(const char* exp1, const char* exp2) {
    ...
    fault_AddClient(&client, (void*)fault_HungupFaultClient, (void*)exp1, (void*)exp2);
