...
class N64SegEvw_colprim(CommonSegCodeSubsegment):
    ...
    def scan(self, rom_bytes: bytes):
        ...
        if (len(data)) != 5:
            log.error(f"Error: evw_colprim segment {self.name} length ({len(data)}) is not 5 bytes!")

        if not self.data_only:
            lines.append(options.opts.generated_c_preamble)
            lines.append("\n")
            lines.append(f"EvwAnimeColPrim {symbol.name} = ")

        r, g, b, a, l = struct.unpack(">bbbbb", data)
