...
class N64SegCkf_je(CommonSegCodeSubsegment):
    ...
    def disassemble_data(self, rom_bytes) -> str:
        ...
        jointelem_data = rom_bytes[self.rom_start : self.rom_end]
        segment_length = len(jointelem_data)
        if (segment_length) % 12 != 0:
            log.error(
                f"Error: ckf_je segment {self.name} length ({segment_length}) is not a multiple of 12!"
            )
        ...
        for jointelem in struct.iter_unpack(">IBBhhh", jointelem_data):
            shape, numberOfChildren, displayBufferFlag, x, y, z = jointelem
            jointelem_string = f"    {{ {shape:08X}, {numberOfChildren}, {displayBufferFlag}, {{{x}, {y}, {z}}} }},"
            lines.append(jointelem_string)
