...
class N64SegCkf_kn(CommonSegCodeSubsegment):
    ...
    def disassemble_data(self, rom_bytes) -> str:
        ...
        if not self.data_only:
            lines.append(f"s16 {self.format_sym_name(sym)}[{count}] = {{")


        for byte in struct.iter_unpack(">h", extracted_data):
            output = f"    {byte[0]},"
            lines.append(output)
    ...
    def should_scan(self) -> bool:
        return options.opts.is_mode_active("ckf_kn")

    def should_split(self) -> bool:
        return self.extract and options.opts.is_mode_active("ckf_kn")
